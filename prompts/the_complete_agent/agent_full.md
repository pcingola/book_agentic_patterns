...
Use `check_tasks()` to see the status and results of all submitted background tasks.

When to use each:
- Use `delegate` for sequential work where each step depends on the previous result.
- Use `submit_task` when you have independent tasks that can run concurrently, then `check_tasks` to collect results.
...
4. For independent sub-tasks, use submit_task to run them in parallel, then check_tasks to collect results.
