...
class TestSandboxReadOnlyMounts(unittest.TestCase):
    ...
    def setUp(self):
        ...
        (scripts_dir / "args.py").write_text("import sys; print(' '.join(sys.argv[1:]))")

        self.registry = SkillRegistry()
        self.registry.discover([self.skills_root])

        self.manager = SandboxManager(read_only_mounts={
    ...
    def test_mounted_file_is_readable_inside_container(self):
        exit_code, output = self.manager.execute_command(self.user_id, self.session_id, "cat /skills/my-skill/scripts/hello.py")
        self.assertEqual(exit_code, 0)
        self.assertIn("hello from skill", output)

    def test_mounted_file_is_read_only(self):
        exit_code, _ = self.manager.execute_command(self.user_id, self.session_id, "touch /skills/my-skill/scripts/new_file.py")
        self.assertNotEqual(exit_code, 0)

    def test_run_skill_script_python(self):
        exit_code, output = run_skill_script(self.manager, self.registry, self.user_id, self.session_id, "my-skill", "hello.py")
        self.assertEqual(exit_code, 0)
        self.assertIn("hello from skill", output)

    def test_run_skill_script_shell(self):
        exit_code, output = run_skill_script(self.manager, self.registry, self.user_id, self.session_id, "my-skill", "greet.sh", "world")
        self.assertEqual(exit_code, 0)
        self.assertIn("hi world", output)

    def test_run_skill_script_with_args(self):
        exit_code, output = run_skill_script(self.manager, self.registry, self.user_id, self.session_id, "my-skill", "args.py", "foo bar")
        self.assertEqual(exit_code, 0)
        self.assertIn("foo bar", output)

    def test_run_skill_script_unknown_skill(self):
        exit_code, output = run_skill_script(self.manager, self.registry, self.user_id, self.session_id, "nonexistent", "run.py")
        self.assertEqual(exit_code, 1)
        self.assertIn("not found", output)

    def test_run_skill_script_unknown_script(self):
        exit_code, output = run_skill_script(self.manager, self.registry, self.user_id, self.session_id, "my-skill", "missing.py")
        self.assertEqual(exit_code, 1)
        self.assertIn("not found", output)
