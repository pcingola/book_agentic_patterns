...
class TestSqlConnector(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        config = DbConnectionConfig(
            db_id="test_bookstore", type=DatabaseType.SQLITE, dbname=str(cls.db_path)
        )
        DbConnectionConfigs.get().add(config)

        db_info = DbInfo(
            db_id="test_bookstore", description="Test bookstore", tables=[]
        )
        ...
        books.add_column(
            ColumnInfo(name="author_id", data_type="INTEGER", is_nullable=False)
        )
    ...
    def test_execute_select(self):
        result = asyncio.run(
            self.connector.execute_sql(
                "test_bookstore", "SELECT COUNT(*) as cnt FROM books"
            )
        )
    ...
    def test_execute_select_multiple_rows(self):
        result = asyncio.run(
            self.connector.execute_sql(
                "test_bookstore", "SELECT id, title FROM books LIMIT 3"
            )
        )
    ...
    def test_get_row_by_id(self):
        result = asyncio.run(
            self.connector.get_row_by_id("test_bookstore", "books", "1")
        )
        self.assertIn("title", result)

    def test_get_row_not_found(self):
        result = asyncio.run(
            self.connector.get_row_by_id("test_bookstore", "books", "99999")
        )
        self.assertIn("error", result)

    def test_get_row_with_related(self):
        result = asyncio.run(
            self.connector.get_row_by_id(
                "test_bookstore", "books", "1", fetch_related=True
            )
        )
    ...
    def test_table_not_found(self):
        result = asyncio.run(
            self.connector.get_row_by_id("test_bookstore", "nonexistent", "1")
        )
        self.assertIn("error", result)
