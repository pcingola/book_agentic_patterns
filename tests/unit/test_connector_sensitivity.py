...
from agentic_patterns.core.connectors.openapi.api_connection_config import ApiConnectionConfig, ApiConnectionConfigs
...
"""
        ...
        try:
            ...
            self.assertEqual(configs.get_config("patients_db").sensitivity, DataSensitivity.CONFIDENTIAL)
            self.assertEqual(configs.get_config("public_db").sensitivity, DataSensitivity.PUBLIC)
        finally:
            yaml_path.unlink()
    ...
    def test_api_config_defaults_to_public(self):
        config = ApiConnectionConfig(api_id="test", spec_source="http://example.com/spec", base_url="http://example.com")
        self.assertEqual(config.sensitivity, DataSensitivity.PUBLIC)
...
"""
        ...
        try:
            ...
            configs.load_from_yaml(yaml_path)
            self.assertEqual(configs.get_config("internal_api").sensitivity, DataSensitivity.INTERNAL)
            self.assertEqual(configs.get_config("public_api").sensitivity, DataSensitivity.PUBLIC)
        finally:
            yaml_path.unlink()
