...
There is another class of leakage that session-level tagging does not address on its own: cached data shared across sessions. When a connector retrieves private data and the result -- or the connection used to retrieve it -- is cached at the process level, a different user's request can hit that cache and receive data it was never authorized to see. The tagging logic fires inside the connector, but a cache hit can bypass the connector entirely.

The rule is straightforward. Caches holding data from restricted resources must be keyed by `(user_id, session_id)` -- the same scoping used for workspaces and private-data tags. Global caches keyed only by resource ID are safe for public, static metadata (database schemas, vocabulary terms, API specifications) but unsafe for query results, document contents, or anything behind access control. The Todo toolkit's cache (`toolkits/todo/operations.py`), which is keyed by `(user_id, session_id)`, is a correct reference for this pattern.

Consider an MCP SQL server running as a shared process. The `SqlConnector` delegates to `DbInfos.get()`, which caches database connections by `db_id`. All users share the same connection object. Today these connections use service credentials configured in `config.yaml`, so access levels are uniform across users. But if credentials were per-user -- OAuth tokens, row-level security contexts, connection-level authorization -- the global cache becomes a credential leak. User A authenticates, the connection is cached, and User B's request reuses it with User A's privileges.

The same concern applies to vector databases. `get_vector_db()` caches collections by name with no user scoping. If User A ingests confidential documents into a shared collection, User B querying that collection retrieves those chunks. Either scope collections per user (by including the user ID in the collection name) or enforce access-control filtering at retrieval time.

Module-level connector instances in MCP servers present a subtler version of the same problem. A line like `_sql = SqlConnector()` at module scope creates a single instance shared across all requests to that server. Even if connectors are currently stateless, this pattern normalizes global mutable state and makes it easy for future changes to introduce per-request data that leaks across sessions. Instantiating connectors per request is cheap and makes the per-request scope explicit.

Identity propagation (covered in the UI chapter's session propagation section) is a necessary precondition for cache isolation -- you cannot scope a cache by user if you do not know who the user is. But propagating identity is not sufficient on its own. Every cache between the identity boundary and the data source must also respect that identity, or the propagation is bypassed by the first cache hit.
