...
**user_session.py**: User and session context extraction from RunContext (`get_user_id_from_request()`, `get_session_id_from_request()`).

**workspace.py**: Sandbox workspace isolation for multi-tenant scenarios. `container_to_host_path()` and `host_to_container_path()` translate between agent-visible paths (`/workspace/...`) and host filesystem paths with traversal protection. File operations: `read_from_workspace()`, `write_to_workspace()`, `store_result()`.
