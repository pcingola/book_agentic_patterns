...
**connectors/**: Data source connectors for agent file operations. `FileConnector` (file.py) provides read, write, append, delete, edit, find, head, tail, list operations. `CsvConnector` (csv.py) provides append, delete, query, read with auto-delimiter detection. `JsonConnector` (json.py) provides JSONPath-based read/write with structure truncation. All connectors use static methods, `@tool_permission()` decorators (READ/WRITE), and workspace sandbox isolation via `container_to_host_path()`. **Vocabulary sub-connector** (`connectors/vocabulary/`): `VocabularyConnector` provides term resolution across controlled vocabularies and ontologies. Three resolution strategies based on vocabulary size: `StrategyEnum` (< 100 terms, in-memory dict with exact/fuzzy lookup), `StrategyTree` (< 1K terms, adjacency list with BFS/DFS traversal for hierarchy queries), `StrategyRag` (1K+ terms, vector DB via existing `vectordb` module for semantic search). Data models in `models.py` (`VocabularyTerm`, `VocabularyInfo`, `VocabularyConfig`, `VocabularyStrategy`). `registry.py` manages vocabulary registration and routing. Connector methods: lookup, search, validate, suggest, parent, children, siblings, ancestors, descendants, roots, relationships, related, subtree, info, list_vocabularies. Toy data in `toy_data.py` for testing. Config in `config.py` with `VOCABULARIES_YAML_PATH`. **SQL sub-connector** (`connectors/sql/`): Pydantic data models for schema metadata (`DbInfo`, `TableInfo`, `ColumnInfo`, `ForeignKeyInfo`, `IndexInfo`), abstract + SQLite implementations for connections (`DbConnection`, `DbConnectionSqlite`), schema inspection (`DbSchemaInspector`, `DbSchemaInspectorSqlite`), schema extraction with JSON caching (`DbSchemaExtractor`), database operations (`DbOperations`, `DbOperationsSqlite`), business logic operations layer (`operations.py` with `db_execute_sql_op`, `db_get_row_by_id_op`, `db_list_op`, etc.), query validation (SELECT-only, single statement), schema SQL formatting (`SchemaFormatter`), singleton registries (`DbConnectionConfigs`, `DbInfos`), factory functions with match/case dispatch on `DatabaseType`. Configuration in `connectors/sql/config.py` reads from `dbs.yaml`. Schema annotation pipeline (`annotation/`) with AI-powered descriptions, enum detection, and example query generation. NL2SQL agent (`nl2sql/`) creates PydanticAI agents with database-bound tools via closure pattern. DB catalog agent (`db_catalog/`) selects the appropriate database for a query. Prompt templates in `prompts/sql/`. Architecture supports adding Postgres via new subdirectories and factory match/case branches.
...
- `annotate_schema.sh` - Runs AI schema annotation pipeline (`python -m agentic_patterns.core.connectors.sql.cli.annotate_schema`)
