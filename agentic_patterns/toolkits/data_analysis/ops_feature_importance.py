    ...
    for col in non_numeric_cols:
        try:
            if pd.api.types.is_datetime64_any_dtype(x[col]):
                ...
                sample_value = (
                    x[col].dropna().iloc[0] if not x[col].dropna().empty else None
                )
                if sample_value is not None and isinstance(
                    sample_value, list | dict | tuple | set
                ):
            ...
            raise UnencodableColumnError(
                f"Failed to encode column '{col}': {e}", col
            ) from e
...
def gradient_boosting_feature_importance(
    ...
    try:
        ...
        raise ValueError(
            f"Failed to fit gradient boosting model (model_type={model_type!r}): {e}"
        ) from e
...
def linear_feature_importance(
    ...
    if not pd.api.types.is_numeric_dtype(y):
        try:
            ...
            raise ValueError(
                f"Target column '{target}' contains non-numeric data that cannot be converted: {e}"
            ) from e

    non_numeric = [col for col in features if not pd.api.types.is_numeric_dtype(x[col])]
    if non_numeric:
        raise ValueError(
            f"Feature columns {non_numeric} contain non-numeric data. Preprocess before using linear feature importance."
        )
...
def permutation_feature_importance_fn(
    ...
    try:
        ...
        raise ValueError(
            f"Failed to fit model for model_type={model_type!r}: {e}"
        ) from e
...
def random_forest_feature_importance(
    ...
    try:
        ...
        raise ValueError(
            f"Failed to fit random forest model (model_type={model_type!r}): {e}"
        ) from e
