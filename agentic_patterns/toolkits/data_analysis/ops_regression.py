...
from agentic_patterns.toolkits.data_analysis.ml_helpers import train_and_fit_regressor
...
REGRESSION_OPERATIONS = {
    "linear_regression": OperationConfig(
        ...
        func=lambda df, target, features=None, test_size=0.2, **kwargs: (
            train_and_fit_regressor(
                df,
                target,
                features,
                test_size,
    ...
    "ridge_regression": OperationConfig(
        ...
        category="ml_regression",
        func=lambda df, target, features=None, test_size=0.2, alpha=1.0, random_state=42, **kwargs: (
            train_and_fit_regressor(
                df,
                target,
                features,
    ...
    "lasso_regression": OperationConfig(
        ...
        func=lambda df, target, features=None, test_size=0.2, alpha=1.0, random_state=42, **kwargs: (
            train_and_fit_regressor(
                ...
                features,
                test_size,
                Lasso(alpha=alpha, random_state=random_state, **kwargs),
                random_state=random_state,
            )
        ),
        parameters={
    ...
    "knn_regression": OperationConfig(
        ...
        func=lambda df, target, features=None, test_size=0.2, n_neighbors=5, **kwargs: (
            train_and_fit_regressor(
                ...
                features,
                test_size,
                KNeighborsRegressor(n_neighbors=n_neighbors, **kwargs),
            )
        ),
        parameters={
            "target": str,
    ...
    "svr_regression": OperationConfig(
        name="svr_regression",
        category="ml_regression",
        func=lambda df, target, features=None, test_size=0.2, kernel="rbf", **kwargs: (
            train_and_fit_regressor(
                df,
                target,
                features,
                test_size,
                SVR(kernel=kernel, **kwargs),
            )
        ),
