...
from agentic_patterns.core.config.config import CHAINLIT_DATA_LAYER_DB, CHAINLIT_SCHEMA_FILE
        ...
        if not tables:
            ...
            for statement in sql_content.split(';'):
                ...
                if statement:
                    ...
                    statement_sqlite = statement.replace('UUID', 'TEXT').replace('JSONB', 'TEXT').replace('TEXT[]', 'TEXT')
...
def get_sqlite_data_layer(db_path: Path = CHAINLIT_DATA_LAYER_DB) -> SQLAlchemyDataLayer:
    ...
    return SQLAlchemyDataLayer(conninfo=connection_string, storage_provider=FilesystemStorageClient())
