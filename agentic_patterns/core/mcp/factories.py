...
from agentic_patterns.core.mcp.config import MCPClientConfig, MCPServerConfig, load_mcp_settings
from agentic_patterns.core.mcp.middleware import AuthSessionMiddleware
from agentic_patterns.core.mcp.servers import MCPServerPrivateData, MCPServerStrict, default_mcp_log_handler
...
def get_mcp_client(name: str, config_path: Path | str | None = None, bearer_token: str | None = None) -> MCPServerStrict | MCPServerPrivateData:
    ...
    common: dict[str, Any] = dict(timeout=config.read_timeout, headers=headers, log_handler=default_mcp_log_handler)
    if config.url_isolated:
        return MCPServerPrivateData(url=config.url, url_isolated=config.url_isolated, **common)
    return MCPServerStrict(url=config.url, **common)


def get_mcp_clients(names: list[str], config_path: Path | str | None = None) -> list[MCPServerStrict | MCPServerPrivateData]:
