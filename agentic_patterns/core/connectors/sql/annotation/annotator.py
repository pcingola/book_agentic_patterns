...
from agentic_patterns.core.connectors.sql.annotation.example_queries import (
    generate_example_queries,
)
from agentic_patterns.core.connectors.sql.config import MAX_SAMPLE_ROWS
from agentic_patterns.core.connectors.sql.db_info import DbInfo
from agentic_patterns.core.connectors.sql.inspection.schema_extractor import (
    DbSchemaExtractor,
)
...
class DbSchemaAnnotator:
    ...
    async def _annotate_db_description(self, db_info: DbInfo, verbose: bool) -> None:
        ...
        prompt = get_prompt(
            "sql/schema_annotation/schema_db_description", schema=schema
        )
    ...
    async def _annotate_table_descriptions(
        self, db_info: DbInfo, verbose: bool
    ) -> None:
        for table in db_info.tables:
            table_schema = table.schema_sql()
            prompt = get_prompt(
                "sql/schema_annotation/schema_table_description",
    ...
    async def _annotate_column_descriptions(
        self, db_info: DbInfo, verbose: bool
    ) -> None:
        for table in db_info.tables:
            table_schema = table.schema_sql()
            prompt = get_prompt(
                "sql/schema_annotation/schema_columns_description",
                db_description=db_info.description,
                table_schema=table_schema,
            )
            agent = get_agent(system_prompt="You are a database analyst.")
        ...
        for table in db_info.tables:
            try:
                df = await db_ops.execute_select_query(
                    f'SELECT * FROM "{table.name}" LIMIT {MAX_SAMPLE_ROWS}'
                )
