...
from agentic_patterns.core.connectors.openapi.client.requests_client import (
    RequestsApiClient,
)
...
def create_spec_parser(
    spec_dict: dict, api_id: str, base_url: str | None = None
) -> ApiSpecParser:
    ...
    if "openapi" in spec_dict:
        ...
        match version:
            ...
            case _:
                raise ValueError(
                    f"Unsupported OpenAPI version: {version}. Only OpenAPI 3.x is supported."
                )
    elif "swagger" in spec_dict:
        swagger_version = spec_dict["swagger"]
        raise ValueError(
            f"Swagger {swagger_version} is not supported. Only OpenAPI 3.x is supported."
        )
    else:
        raise ValueError(
            "Unable to determine API spec format. Missing 'openapi' or 'swagger' field."
        )
