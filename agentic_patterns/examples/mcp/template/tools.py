"""Template MCP server tools demonstrating requirements 1-8.

Each tool shows a different requirement:
- read_file: @tool_permission(READ), @context_result(), workspace path translation
- write_file: @tool_permission(WRITE), workspace path translation
- search_records: @tool_permission(READ), ToolRetryError for bad input
- load_sensitive_dataset: @tool_permission(READ), PrivateData flagging, ToolFatalError
"""
...
def register_tools(mcp: FastMCP) -> None:
    ...
    @mcp.tool()
    @tool_permission(ToolPermission.READ)
    def load_sensitive_dataset(dataset_name: str) -> str:
