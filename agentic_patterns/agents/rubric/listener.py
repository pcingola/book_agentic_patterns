...
class RubricListener:
    ...
    async def on_group_start(
        self, pass_num: int, group_num: int, n_groups: int
    ) -> None:
        pass

    async def on_group_done(
        self,
        pass_num: int,
        n_groups_done: int,
        n_groups_total: int,
    ...
    async def on_item_added(
        self, item_id: str, title: str, requirement_level: str
    ) -> None:
        pass
...
class PrintRubricListener(RubricListener):
    ...
    async def on_pass_start(self, pass_num: int, n_items: int) -> None:
        if pass_num == -1:
            print(
                f"[rubric] extracting requirements from {n_items} document chunks (in parallel)"
            )
        elif pass_num == 0:
            print(f"[rubric] synthesis: mapping {n_items} candidates into rubric items")
        else:
            print(
                f"[rubric] merge pass {pass_num}: reducing {n_items} candidates by semantic similarity"
            )
    ...
    async def on_group_done(
        ...
        if pass_num == -1:
            print(
                f"[rubric]   chunk {n_groups_done}/{n_groups_total}: {len(items or [])} extracted"
            )
...
class PrintRubricEvaluatorListener(RubricEvaluatorListener):
    ...
    async def on_done(self, verdicts: list[RubricVerdict]) -> None:
        counts = {
            s: sum(1 for v in verdicts if v.status.value == s)
            for s in ("PASS", "RISK", "FAIL")
        }
