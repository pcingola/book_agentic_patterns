...
class DeepResearchAgent:
    ...
    async def _decompose(self, question: str) -> list[str]:
        ...
        prompt = load_prompt(RESEARCH_PROMPTS / "decompose.md", question=question, max_questions=self._max_questions)
    ...
    async def run(self, question: str) -> ResearchReport:
        ...
        for iteration in range(1, self._max_iterations + 1):
            ...
            if assessment.sufficient:
                break
            logger.info("Iteration %d: %d gaps found, re-querying", iteration, len(assessment.gaps))
